...
namespace MarketPlaceForYou.Services.Services
{
    public class ListingService : IListingService
    {
        ...
        {
            var results = await _uow.Listings.GetAll(items => items.Where(items => items.UserId != userId).Include(items => items.User).Include(items => items.Upload.Url));
        ...
        {
            var results = await _uow.Listings.GetAll(items => items.Where(items => items.City == city && items.UserId != userid).Include(items => items.User).Include(items => items.User).Include(items => items.Upload.Url));
            var models = results.Select(listing => new ListingVM(listing)).ToList();
            return models;
        }
        public async Task<List<ListingVM>> GetAllByCategory(string category, string userid)
        {
            var results = await _uow.Listings.GetAll(items => items.Where(items => items.Category == category && items.UserId != userid).Include(items => items.User).Include(items => items.Upload.Url));
            var models = results.Select(listing => new ListingVM(listing)).ToList();
            return models;
        }
        public async Task<List<ListingVM>> Search(string searchString, string userid)
        {
            var results = await _uow.Listings.GetAll(items => items.Where(items => (items.Description.ToLower().Contains(searchString.ToLower()) || items.ProdName.ToLower().Contains(searchString.ToLower())) && items.UserId != userid).Include(items => items.User).Include(items => items.Upload.Url));
            var models = results.Select(listing => new ListingVM(listing)).ToList();
            return models;
        }
        //Needs work
        public async Task<List<ListingVM>> SearchWithFilters(string userid, string? searchString = null, string? city = null, string? category = null, string? condition = null, decimal minPrice = 0, decimal maxPrice = 0)
        {
            ...
            var query = await _uow.Listings.GetAll().AsQueryable();
            ...
            //var results = await _uow.Listings.GetAll(items => items.Where(items => items.);
        ...
        {
            var results = await _uow.Listings.GetAll(items => items.Where(items => items.UserId == userid && items.Status == "Acive").Include(items => items.User).Include(items => items.Upload.Url));
        ...
        {
            var results = await _uow.Listings.GetAll(items => items.Where(items => items.UserId == userid && items.Status == "Sold").Include(items => items.User).Include(items => items.Upload.Url));
            var model = results.Select(listings => new ListingVM(listings)).ToList();
            return model;
        }
        public async Task<List<ListingVM>> MyPurchases(string userId)
        {
            var results = await _uow.Listings.GetAll(items => items.Where(items => items.BuyerID == userId).Include(items => items.User).Include(items => items.Upload.Url));
            var model = results.Select(listings => new ListingVM(listings)).ToList();
            return model;
        }
        public async Task<List<ListingVM>> PendingListings(string userId)
        {
            var results = await _uow.Listings.GetAll(Items => Items.Where(items => items.UserId == userId && items.Status == "Pending").Include(items => items.User).Include(items => items.Upload.Url));
            var model = results.Select(listings => new ListingVM(listings)).ToList();
            return model;
        }
        ...
        {
            var entity = await _uow.Listings.GetById(src.Id, items => items.Include(items => items.User).Include(items => items.Upload.Url));
        ...
        {
            var entity = await _uow.Listings.GetById(id, items => items.Include(items => items.User).Include(items => items.Upload.Url));
